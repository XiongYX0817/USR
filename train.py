...
from ibrnet.criterion import Criterion, normal_loss, segmentation_loss
...
def train(args):
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            featmaps = model.feature_net(ray_batch['src_rgbs'].squeeze(0).permute(0, 3, 1, 2).to(device))
            ...
            if args.seg_weight > 0:
                coarse_seg_loss = segmentation_loss(ret['outputs_coarse']['T'].reshape(-1).detach(), ret['outputs_coarse']['confidence'], ray_batch['segmentation'])
                loss += args.seg_weight * coarse_seg_loss
                scalars_to_log['coarse_seg_loss'] = coarse_seg_loss.item()
            ...
            if ret['outputs_fine'] is not None:
                ...
                if args.seg_weight > 0:
                    fine_seg_loss = segmentation_loss(ret['outputs_fine']['T'].reshape(-1).detach(), ret['outputs_fine']['confidence'], ray_batch['segmentation'])
                    loss += args.seg_weight * fine_seg_loss
                    scalars_to_log['fine_seg_loss'] = fine_seg_loss.item()

            # for key in ret['outputs_fine']:
            #     print(key)
            # print(ret["pts_fine"].shape)
            # print(ret['outputs_fine']['alpha'].shape)
            # exit()

            if args.grad_weight > 0: # and global_step >= 1000:
                ranking = torch.argsort(ret['outputs_fine']['weights'], dim=1)
                surface_idxes = ranking[:, -3:]
                surface_pts = torch.cat([ret["pts_fine"][i][surface_idxes[i]] for i in range(ret["pts_fine"].shape[0])], dim=0)
                # surface_pts_alpha = torch.cat([ret['outputs_fine']['alpha'][i][surface_idxes[i]] for i in range(ret["pts_fine"].shape[0])], dim=0)
                # surface_pts_weights = torch.cat([ret['outputs_fine']['alpha'][i][surface_idxes[i]] for i in range(ret["pts_fine"].shape[0])], dim=0)
                # print(surface_pts_alpha)
                # print(surface_pts_weights)
                # exit()
                loss_norm = normal_loss(model, projector, ray_batch, surface_pts, featmaps)
                # loss += args.grad_weight * loss_norm
                scalars_to_log['loss_norm'] = loss_norm.item()

            loss.backward()
            scalars_to_log['loss'] = loss.item()
            # # torch.nn.utils.clip_grad_norm(model.net_coarse.parameters(), 1.0, norm_type=2)
            # print("==================================================================================")
            # print("[INFO] Grad 1")
            for p in model.learnable_params:
                if p.grad is not None:
                    grad_nan_position = torch.isnan(p.grad)
                    grad_inf_position = torch.isinf(p.grad)
                    p.grad[grad_nan_position] = 0
                    p.grad[grad_inf_position] = 0
            for net in [model.feature_net, model.net_coarse, model.net_fine]:
                for name, parameters in net.named_parameters():
                    if parameters.grad is not None:
                        if torch.isnan(parameters.grad).any():
                            exit(name + " nan")
                        if torch.isinf(parameters.grad).any():
                            print(name + " inf")
            # torch.nn.utils.clip_grad_norm_(model.feature_net.parameters(), 1.0, norm_type=2)
            # print("[INFO] Grad 2")
            # for name, parameters in model.feature_net.named_parameters():
            #     if name == "conv1.weight":
            #         print(parameters.grad)
            # print("==================================================================================")
            # input()
            # # if model.net_fine is not None
            # #     torch.nn.utils.clip_grad_norm(model.net_fine.parameters(), 1.0, norm_type=2)
            ...
            if args.local_rank == 0:
                ...
                if global_step % args.i_img == 0:
                    ...
                    log_view_to_tb(writer, global_step, args, model, tmp_ray_sampler, projector,
                                   gt_img, out_folder, render_stride=args.render_stride, prefix='val/')
                    ...
                    log_view_to_tb(writer, global_step, args, model, tmp_ray_train_sampler, projector,
                                   gt_img, out_folder, render_stride=1, prefix='train/')
            global_step += 1
...
def log_view_to_tb(writer, global_step, args, model, ray_sampler, projector, gt_img, out_folder,
    ...
    if not os.path.exists(out_folder+"/display/"):
        os.mkdir(out_folder+"/display/")
    rgb_im = rgb_im.permute(1, 2, 0) * 255
    if prefix.startswith('val'):
        imageio.imwrite(out_folder+f'/display/val_img_{global_step}.png', rgb_im.numpy().astype(np.uint8))
    else:
        imageio.imwrite(out_folder+f'/display/train_img_{global_step}.png', rgb_im.numpy().astype(np.uint8))
