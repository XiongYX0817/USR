...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1):
        ...
        self.segmentation = data['segmentation'] if 'segmentation' in data.keys() else None
        ...
        if self.segmentation is not None:
            self.segmentation = self.segmentation.reshape(-1)
    ...
    def get_all(self):
        ret = {'ray_o': self.rays_o.cuda(),
               ...
               'segmentation': self.segmentation.cuda() if self.segmentation is not None else None,
    ...
    def random_sample(self, N_rand, sample_mode, center_ratio=0.8):
        ...
        if self.segmentation is not None:
            segmentation = self.segmentation[select_inds]
        else:
            segmentation = None
        ...
        ret = {'ray_o': rays_o,
               ...
               'segmentation': segmentation.to(self.device) if segmentation is not None else None,
