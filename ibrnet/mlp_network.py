...
class IBRNet(nn.Module):
    def __init__(self, args, in_feat_ch=32, n_samples=64, **kwargs):
        ...
        self.seg_fc = nn.Sequential(nn.Linear(32, 32),
                                    activation_func,
                                    nn.Linear(32, 33),
                                    activation_func,
                                    )

        self.seg_fc2 = nn.Sequential(nn.Linear(32, 32),
                                     activation_func,
                                     nn.Linear(32, 1),
                                     nn.Sigmoid()
                                     )
        ...
        # self.out_segmentation_fc = nn.Sequential(nn.Linear(32*2+1, 64),
        #                                          activation_func,
        #                                          nn.Linear(64, 16))

        out_segmentation_fc_blocks = [nn.Linear(32*2+1, 64)]
        for i in range(args.seg_fc_hidden_layers):
            out_segmentation_fc_blocks.append(nn.Linear(64, 64))
        out_segmentation_fc_blocks.append(nn.Linear(64, 16))
        self.out_segmentation_fc = nn.Sequential(*out_segmentation_fc_blocks)
        ...
        self.seg_fc2.apply(weights_init)
        self.seg_fc.apply(weights_init)
        self.geometry_fc.apply(weights_init)
        self.out_geometry_fc.apply(weights_init)
        self.out_segmentation_fc.apply(weights_init)
    ...
    def forward(self, rgb_feat, ray_diff, mask, with_sigmoid=True):
        '''
        ...
        n_rays, n_samples, num_views = rgb_feat.shape[0], rgb_feat.shape[1], rgb_feat.shape[2]
        ...
        x_vis_tmp = self.vis_fc(x * weight)
        x_res, vis = torch.split(x_vis_tmp, [x_vis_tmp.shape[-1]-1, 1], dim=-1)
        vis = F.sigmoid(vis) * mask
        x_vis = x + x_res
        vis = self.vis_fc2(x_vis * vis) * mask
        weight_vis = vis / (torch.sum(vis, dim=2, keepdim=True) + 1e-8)

        mean, var = fused_mean_variance(x_vis, weight_vis)
        globalfeat = torch.cat([mean.squeeze(2), var.squeeze(2), weight_vis.mean(dim=2)], dim=-1)  # [n_rays, n_samples, 32*2+1]
        density_feat = self.geometry_fc(globalfeat)  # [n_rays, n_samples, 16]
        ...
        sigma = self.out_geometry_fc(density_feat)  # [n_rays, n_samples, 1]
        # sigma_out = sigma.masked_fill(num_valid_obs < 1, 0.)  # set the sigma of invalid point to zero
        sigma_out = self.sigmoid(sigma * 10.0) if with_sigmoid else sigma

        x_seg_tmp = self.seg_fc(x * weight)
        x_res, vis = torch.split(x_seg_tmp, [x_seg_tmp.shape[-1]-1, 1], dim=-1)
        vis = F.sigmoid(vis) * mask
        x_seg = x + x_res
        vis = self.seg_fc2(x_seg * vis) * mask
        weight_seg = vis / (torch.sum(vis, dim=2, keepdim=True) + 1e-8)

        mean, var = fused_mean_variance(x_seg, weight_seg)
        globalfeat = torch.cat([mean.squeeze(2), var.squeeze(2), weight_seg.mean(dim=2)], dim=-1)  # [n_rays, n_samples, 32*2+1]
        confidence = self.out_segmentation_fc(globalfeat)
        ...
        out = torch.cat([rgb_out, sigma_out, confidence], dim=-1)
        return out
