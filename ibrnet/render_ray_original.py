...
def render_rays(ray_batch,
    ...
    if N_importance > 0:
        ...
        ret['z_vals_fine'] = z_vals

    return ret

def render_rays_batch(ray_batch,
                model,
                featmaps,
                projector,
                N_samples,
    ...
    rgb_feat, ray_diff, mask, pixel_mask = [], [], [], []
    for i in range(N_rays):
        rgb_feat_, ray_diff_, mask_ = projector.compute(pts[i: i+1], ray_batch['camera'][i: i+1],
                                                     ray_batch['src_rgbs'],
                                                     ray_batch['src_cameras'],
                                                     featmaps=featmaps[0])  # [N_rays, N_samples, N_views, x]
        # import pickle
        # with open("tmp.pickle", "wb") as f:
        #     pickle.dump([ray_diff_.detach().cpu(), pts.detach().cpu()], f)
        # exit()
        pixel_mask_ = mask_[..., 0].sum(dim=2) > 1   # [N_rays, N_samples], should at least have 2 observations
        rgb_feat.append(rgb_feat_)
        ray_diff.append(ray_diff_)
        mask.append(mask_)
        pixel_mask.append(pixel_mask_)
    rgb_feat = torch.cat(rgb_feat, dim=0)
    ray_diff = torch.cat(ray_diff, dim=0)
    ...
    if N_importance > 0:
        ...
        rgb_feat_sampled, ray_diff, mask, pixel_mask = [], [], [], []
        for i in range(N_rays):
            rgb_feat_sampled_, ray_diff_, mask_ = projector.compute(pts[i: i+1], ray_batch['camera'][i: i+1],
                                                                 ray_batch['src_rgbs'],
                                                                 ray_batch['src_cameras'],
                                                                 featmaps=featmaps[1])
            pixel_mask_ = mask_[..., 0].sum(dim=2) > 1  # [N_rays, N_samples]. should at least have 2 observations
            rgb_feat_sampled.append(rgb_feat_sampled_)
            ray_diff.append(ray_diff_)
            mask.append(mask_)
            pixel_mask.append(pixel_mask_)
        rgb_feat_sampled = torch.cat(rgb_feat_sampled, dim=0)
        ray_diff = torch.cat(ray_diff, dim=0)
        mask = torch.cat(mask, dim=0)
        pixel_mask = torch.cat(pixel_mask, dim=0)
        raw_fine = model.net_fine(rgb_feat_sampled, ray_diff, mask)
