...
class MyDataset(Dataset):
    def __init__(self, args, subset, scenes=None, **kwargs):

        self.folder_path = os.path.join(args.rootdir, 'data/mydataset/')
        ...
        if subset == "getmesh":
            subset = "test"
        if not scenes:
            for root, dirs, files in os.walk(os.path.join(self.folder_path, subset)):
                self.scenes = sorted(dirs)
                break
        ...
        for scene in self.scenes:
            ...
            seg_files = [f.replace('rgb', 'seg') for f in rgb_files]
            ...
            self.all_seg_files.extend(seg_files)
    ...
    def __getitem__(self, idx):
        ...
        seg_file = self.all_seg_files[idx]
        intrinsics_file = self.all_intrinsics_files[idx]
        intrinsics = deepvoxels_parse_intrinsics(intrinsics_file, 512)[0]
        subset = "test" if self.subset == "getmesh" else self.subset

        scene_path = ''
        for tmp in re.findall("/(cg\_[^/]+)/", intrinsics_file):
            scene_path = os.path.join(self.folder_path, subset, tmp)
            break

        # train_rgb_files = sorted(glob.glob(os.path.join(scene_path.replace('/{}/'.format(subset), '/train/'), 'rgb', '*')))
        train_rgb_files = sorted(glob.glob(os.path.join(scene_path, 'rgb', '*')))
        ...
        seg_fig = np.array(imageio.imread(seg_file))
        segmentation = np.zeros((seg_fig.shape[0], seg_fig.shape[1]))
        segmentation[(seg_fig == [0, 0, 0]).sum(axis=-1) == 3] = 0
        segmentation[(seg_fig == [255, 0, 0]).sum(axis=-1) == 3] = 1
        segmentation[(seg_fig == [0, 255, 0]).sum(axis=-1) == 3] = 2
        segmentation[(seg_fig == [0, 0, 255]).sum(axis=-1) == 3] = 3
        segmentation[(seg_fig == [255, 255, 0]).sum(axis=-1) == 3] = 4
        segmentation[(seg_fig == [250, 235, 215]).sum(axis=-1) == 3] = 5
        segmentation = segmentation.astype(np.long)

        # import matplotlib.pyplot as plt
        # img = np.concatenate([np.array(imageio.imread(rgb_file)), seg_fig], axis=1)
        # plt.imshow(img)
        # plt.show()
        # input()
        ...
        if self.subset == "getmesh":
            nearest_pose_ids = np.array(range(20))
        else:
            nearest_pose_ids = np.random.choice(range(60), 20, replace=False)
...
class MyRenderDataset(Dataset):
    def __init__(self, args, scenes, **kwargs):

        self.folder_path = os.path.join(args.rootdir, 'data/mydataset/')
        self.rectify_inplane_rotation = args.rectify_inplane_rotation
        self.subset = "test"  # train / test / validation
        self.num_source_views = args.num_source_views
        self.testskip = args.testskip
        ...
        for scene in self.scenes:
            self.scene_path = os.path.join(self.folder_path, self.subset, scene)
    ...
    def __getitem__(self, idx):
        ...
        scene_path = ''
        for tmp in re.findall("/(cg\_[^/]+)/", intrinsics_file):
            scene_path = os.path.join(self.folder_path, self.subset, tmp)
            break
        ...
        nearest_pose_ids = np.random.choice(range(140), 20, replace=False)
        # nearest_pose_ids = get_nearest_pose_ids(render_pose,
        #                                         train_poses,
        #                                         # num_source_views,
        #                                         20,
